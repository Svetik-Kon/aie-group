...
Дополнительно (HW03) проект был доработан:

- в core.py добавлены новые эвристики качества данных (флаги в compute_quality_flags), 
например: has_constant_columns, has_high_cardinality_categoricals, has_suspicious_id_duplicates;
- команда report в CLI расширена новыми параметрами (title, top-k категорий, пороги пропусков и т.д.), 
которые реально влияют на содержимое report.md и артефакты отчёта.
...
```
Команда overview выводит не только сводку по колонкам, но и quality_flags — 
эвристики качества данных (результат compute_quality_flags из core.py).
...
--max-hist-columns: сколько числовых колонок включать в набор гистограмм (ограничивает количество файлов hist_*.png).
--top-k-categories: сколько top-значений сохранять для категориальных признаков (влияет на таблицы в top_categories/).
--title: заголовок отчёта (первая строка report.md).
--min-missing-share: порог доли пропусков (0..1). Колонки с долей пропусков >= порога выводятся
отдельным списком в report.md. Если такие колонки есть, дополнительно может сохраняться problem_missing.csv.

Пример запуска (PowerShell / Windows — без символа , лучше в одну строку):
uv run eda-cli report data/example.csv --out-dir reports_example2 --title "Мой тестовый отчёт" --max-hist-columns 2 --top-k-categories 3 --min-missing-share 0.05
...
Важно: поле flags в ответе формируется на основе compute_quality_flags, 
поэтому будет содержать все добавленные эвристики качества данных 
(например has_constant_columns, has_high_cardinality_categoricals, has_suspicious_id_duplicates и т.д.).

...
# ---------- NEW: /quality-flags-from-csv (HW04 2.3.2) ----------


class QualityFlagsResponse(BaseModel):
    """Ответ для эндпоинта, который возвращает полный набор булевых флагов качества."""
    flags: dict[str, bool] = Field(
        ...,
        description="Словарь булевых флагов качества (включая добавленные в HW03).",
    )
...
@app.post(
    ...
    tags=["quality"],
    summary="Вернуть полный набор флагов качества по CSV (включая флаги из HW03)",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт из HW04 (2.3.2):
    - принимает CSV (как /quality-from-csv),
    - вызывает summarize_dataset + missing_table + compute_quality_flags,
    - возвращает только булевы флаги в формате {"flags": {...}}.
    """
    ...
    # Возвращаем ВСЕ булевы флаги (включая те, что ты добавила в HW03)
    flags_bool: dict[str, bool] = {
        k: bool(v)

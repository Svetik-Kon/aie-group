...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for c in summary.columns:
        if c.non_null == 0:
            continue
        if c.is_numeric:
            continue  # считаем "категориальными" только нечисловые

        unique_share = c.unique / max(1, c.non_null)
        if c.unique >= high_card_min_unique and unique_share >= high_card_unique_share:
    ...
    for c in summary.columns:
        if c.non_null == 0:
            continue
        name = c.name.lower()
        looks_like_id = (name == "id") or name.endswith("_id") or name.startswith("id_") or (" id" in name) or (
                    "id " in name)
        if looks_like_id:
            # если уникальных меньше, чем непустых — есть дубликаты
            if c.unique < c.non_null:
                id_dup_cols.append(c.name)
